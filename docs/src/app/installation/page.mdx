...
agent-browser install  # Install Patchright browser artifacts (first time)
```

This is the fastest option. Commands run through the native Rust CLI directly with sub-millisecond parsing overhead.

The default Patchright backend requires Node.js and npm for `agent-browser install`
and for runtime launch. pnpm is only needed when building from source.

## Quick start (no install)

```bash
npx agent-browser-priv install   # Install Patchright browser artifacts
npx agent-browser-priv open example.com
```
...
Patchright updates are controlled by the installed `agent-browser` release. After upgrading `agent-browser`, run `agent-browser install` to refresh the Patchright backend to the version pinned by that release.
...
<tr><td>Browser backend</td><td>Selected local backend install status, including default Patchright or explicit <code>--backend chrome</code>. For Patchright, also reports installed version, release pin, and npm latest when network checks are enabled.</td></tr>
...
<tr><td>Launch test</td><td>Spawns a scratch session, launches a headless browser, navigates to <code>about:blank</code>, then closes. Measures wall time (skipped under <code>--quick</code>)</td></tr>
</tbody>
</table>
...
<tbody>
<tr><td>Patchright missing</td><td>Runs <code>agent-browser install</code></td></tr>
