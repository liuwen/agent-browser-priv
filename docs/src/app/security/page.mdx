...
Local browser providers may need a process that outlives one plugin invocation. The optional [Patchright provider](/providers/patchright) uses private runtime files and a nonce-authenticated loopback control endpoint to close its detached supervisor. Its cleanup token is opaque to agent-browser and the nonce is not included in public browser metadata.
