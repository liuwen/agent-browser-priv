...
The optional Patchright provider is a local example of the same contract:
...
It runs Patchright in a detached supervisor and returns a browser-level loopback CDP endpoint. See [Patchright Provider](/providers/patchright) for its lifecycle, configuration, and driver-boundary limitations.
