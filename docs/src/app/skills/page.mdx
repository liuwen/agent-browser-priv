...
Binary-only installs such as Homebrew hydrate bundled skills into `~/.agent-browser/skills/<version>` when no package directory is available. Use `agent-browser skills path <name>` when an agent needs a real filesystem path.
